...
var car_scene = load("res://car/car_AI_racer.tscn")
...
func _player_disconnected(id):
	if (get_tree().is_network_server()):
		if (has_node("/root/world3D/world2")): # Game is in progress
...
remote func pre_start_game(spawn_points):
	...
	for p_id in spawn_points:
		#world1.add_child(player)
		var car = car_scene.instance()
		
		car.set_name(str(p_id)) # Use unique ID as node name
		...
		#world.get_node("world/players").add_child(player)
		world2.add_child(car)
		
	# Set up score
	#world.get_node("world/score").add_player(get_tree().get_network_unique_id(), player_name)
	for pn in players:
